...
def countall
    puts " combien il y a t'il de journaliste ?"
    counting = $journaliste.length
    puts "#{counting} journaliste"
end


def shortname
    puts " quel est le nom du journaliste le plus court ?"
    short = $journaliste.min {|petit,grand| petit.length <=> grand.length}
    puts short   
end


def fivecaract
    puts "Combien y-a-t'il de nom contenant 5 caractères (le @ ne compte pas pour un caractère)"
    five = $journaliste.select {|cinq| cinq.size == 6}.count        
    puts five
end

def majuscule
    puts "Combien commencent par une majuscule apres le handle (première lettre juste après le @) ?"
    upper = $journaliste.select { |word| word.start_with?( /@[[:upper:]]/ ) }
    puts" il y a #{upper.length} nom qui commence pars une majuscule "
    end

def trialphabétique
    puts "Trie la liste de handle par ordre alphabétique."
    trialpha = $journaliste.sort
    puts trialpha
end

def tripetitgrand
    puts "Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)"
    tripegr = $journaliste.sort {|petit,grand| petit.length <=> grand.length}
    puts tripegr
end

def positionspécifique
    puts "Quelle est la position dans l'array de la personne @epenser ?"
    position = $journaliste.index('@epenser')
    puts " #{position} est la position ou se trouve @epenser "
end
...
def taillehandle
   ...
   liste = ($journaliste.sort {|minimum,maximum| minimum.length <=> maximum.length }
    
    
       handle1 = $journaliste.count {|allhandle| allhandle.size == liste}.count
       puts "il y a #{handle1} handle qui possède #{i} caractere "
